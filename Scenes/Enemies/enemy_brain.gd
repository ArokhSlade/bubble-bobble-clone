...
func get_movement_request() -> Vector2:
	...
	var move_intent : Direction = enemy.heading	
	
	if enemy.is_on_floor():
		enemy.is_jumping = false
			
	if enemy.is_jumping :
		move_intent = enemy.heading	
	...
	enemy.update_heading(move_intent)
...
func can_jump_over_wall():
	...
	wall_coords.x += -1 if enemy.heading & Direction.LEFT else 1
	wall_coords.y -= 1
...
func is_gap_ahead():
	...
	floor_cell_in_front_coords.x += -1 if enemy.heading & Direction.LEFT else 1
	return measure_gap_at(floor_cell_in_front_coords, enemy.heading) > 0
	
	
func can_jump_over_gap():
	var gap_coords = get_floor_cell_coords()
	gap_coords.x += -1 if enemy.heading & Direction.LEFT else 1	
	return get_jump_width_in_tiles() > measure_gap_at(gap_coords, enemy.heading)
	


	
func get_floor_cell_coords() -> Vector2i:
	var my_position = tiles.to_local(enemy.global_position)
...
func jump_forward():
	...
	return Direction.UP | enemy.heading

func opposite_direction() -> Direction:
	if (enemy.heading & Direction.LEFT):
