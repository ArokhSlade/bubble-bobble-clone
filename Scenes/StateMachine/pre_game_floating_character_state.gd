...
class_name PreGameFloatingCharacterState
...
func get_out_of_screen_position() -> Vector2 :
	...
	result.y = 0
	result.y -= character.get("LOWER_HALF_HEIGHT") #NOTE(ArokhSlade, 2024 12 28): NO-OP if it doesn't exist
	return result
...
func initiate_float_to_target_position():
	...
	floating_tween = character.create_tween()
	floating_tween.finished.connect(on_float_finished)
	var floating_seconds = get_floating_seconds()
	floating_tween.tween_property(character, "global_position", target_position, floating_seconds)
